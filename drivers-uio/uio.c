...
*/
static ssize_t uio_read(struct file *filep, char __user *buf,
			size_t count, loff_t *ppos, bool dma_mode)
{
	if(dma_mode == true)
	{
		return = dma_read(filep, userbuf,  count, f_pos)	
	}
	else
	{	
		struct uio_listener *listener = filep->private_data;
...
static ssize_t dma_read(struct file *filp, char __user *userbuf, size_t count, loff_t *f_pos)
{
			...
			size_t count, loff_t *ppos, bool dma_mode)
{
	...
	if(dma_mode == true)
	{
		return dma_write( filp, userbuf, count, f_pos)
	}	
	struct uio_listener *listener = filep->private_data;
	struct uio_device *idev = listener->dev;
	ssize_t retval;
...
static ssize_t dma_write(struct file *filp, const char __user *userbuf, size_t count, loff_t *f_pos)
{
    struct ezdma_drvdata * p_info = (struct ezdma_drvdata*)filp->private_data;
    ssize_t rv = count;
...
struct dma_drvdata {
    ...
    struct dma_inflight_info inflight;
...
struct dma_pdev_drvdata {
    struct list_head dma_list;    // list of ezdma_drvdata instances created in
